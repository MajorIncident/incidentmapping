...
deleted references are not reused. Evidence IDs are globally allocated across
the map as `EV-001`, `EV-002`, and so on. Allocation starts above the greater
of `metadata.evidenceReferenceHighWaterMark` and every well-formed persisted
evidence number, so stale metadata cannot cause reuse. Saving, reopening, and
deleting evidence preserve gaps and the high-water mark.
...
Version 2 parsing is canonical-first: a document that satisfies the canonical
schema is returned with its identities unchanged. Only after canonical parsing
fails is the explicit legacy-V2 compatibility schema attempted. On that legacy
path alone, evidence is deterministically renumbered in node-array then
evidence-array order and the resulting count becomes the evidence high-water
mark. `metadata.status` is the chosen incident-status authority, so retired
node-level `incidentStatus` is accepted but discarded. Retired ActionEdge
